...
class CommentAdapter(val comments:ArrayList<Comment>):RecyclerView.Adapter<CommentAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        val view=LayoutInflater.from(parent.context).inflate(R.layout.comments_item_layout,parent,false)
    ...
    inner class ViewHolder(itemView:View):RecyclerView.ViewHolder(itemView){
        ...
        fun bindComment(comment:Comment){
            setCurrentUserImage(imageProfile)
        ...
        private fun setCurrentUserImage(imageProfile: CircleImageView?) {
            var uid=FirebaseAuth.getInstance().currentUser?.uid
            FirebaseFirestore.getInstance().collection(USER_REF).document(uid.toString()).get().addOnSuccessListener {
